...
const getReadmeFilePaths = async (
    rootPath: string,
    ...
    try {
        const readmeFiles = await fs
            ...
            .then((filesAndFolders) =>
                filesAndFolders.filter((x) => dirNamePattern.test(x)),
            );
        return readmeFiles;
    } catch (error) {
        console.error(
            `[ Error ]: ${(error as Error)?.message || "reading dir"}`,
        );
        console.error(error);
...
const getReadmeContents = async (filePaths: string[]) => {
    try {
        ...
        console.error(
            `[ ERROR - getReadmeContents ]: ${(error as Error).name}`,
        );
        console.error(error);
    }
};
...
const writeToRootReadme = async (
    readmeFilePath: string,
    contentDetails: ContenDetails,
) => {
    try {
        for (const contentDetailsItem of contentDetails) {
        ...
        console.error(
            `[ ERROR - writeToRootReadme ] : ${(error as Error).name}`,
        );
    }
};
...
const generateReadme = async (basePath: string, readmePathPattern: RegExp) => {
    ...
    const readmeFilePaths = await getReadmeFilePaths(
        basePath,
        readmePathPattern,
    );
    ...
    try {
        ...
        console.error("[ ERROR - MAIN ]: failing generating readme", error);
    }
