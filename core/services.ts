...
const getTodayAoCChallenge = async (
    year: string,
    day: string,
): Promise<{ challengeContent: string; inputContent: string }> => {
    try {
        ...
        const contentEndIdx =
            responseText.indexOf("</article>") + "</article>".length;
        ...
        response = await fetch(getTodayInputEndpoint(year, day), {
...
const getTemplateContents = async (): Promise<Record<string, string>> => {
    ...
    try {
        ...
        const textPerFile = splitTexts.reduce<Record<string, string>>(
            (acc, text: string) => {
...
const setupChallengeFolder = async (folderDayPath: string) => {
    ...
    try {
        ...
        if (doesExist) {
            const contentPerFile = await getTemplateContents();

            const files = Object.keys(contentPerFile);
            if (!files.length)
                throw new Error(
                    "Missing content to generate current day files.",
                );
...
export { createChallengeFolder, getTodayAoCChallenge, setupChallengeFolder };
