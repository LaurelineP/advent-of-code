...
const generateYearFolderPath = (year: string | number = CURRENT_YEAR) =>
    `${rootPath}/${year}`;
...
const getGeneratedPath = (argValue: string) => {
    ...
    if (!argValue) pathToExecute = generateDayFolderPath();
        ...
        pathToExecute = getGeneratedPathByYearInput(argValue);
    else if (argValue.length < 4)
        pathToExecute = getGeneratedPathByDayInput(argValue);
